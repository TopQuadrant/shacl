var examples = {
    ...
    example1: {
        ...
        data: JSON.stringify({
            "http://raml.org/vocabularies/shapes#toValidate":
...
QUnit.test("Integration test 1", function (assert) {
    ...
    new SHACLValidator().validate(
        ...
        function (e, report) {
            ...
            assert.ok(results[0].message() === "Less than 1 values");
            assert.ok(results[0].path() === "http://raml.org/vocabularies/shapes/anon#surname");
            assert.ok(results[0].focusNode() !== null);
            assert.ok(results[0].severity() === "Violation");
            assert.ok(results[0].sourceConstraintComponent() === "http://www.w3.org/ns/shacl#MinCountConstraintComponent");
            assert.ok(results[0].sourceShape() === "http://raml.org/vocabularies/data#AnonShape/property/surname");
            done();
        });
});
